...
public class Case20 {
    ...
    public String case20(Event event) {
        ...
        if (
            //habilidades entrenamiento bajo
                ChronoUnit.MONTHS.between(event.getWorker().getEntry(), currentDate) < 6
                     ...
                     //   && !event.getWorkPlace().getInside()
                        && event.getWorkPlace().getWeather().equals(NEBLINA)
            ...
            return "Caso 20 TRABAJO EN ALTURA: habilidades/entrenamiento Baja: La persona tiene: " + ChronoUnit.MONTHS.between(event.getWorker().getEntry(), currentDate) +
